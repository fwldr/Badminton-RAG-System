...
按 `小程序版可行性方案.md`（技术/合规/周期）+ `小程序版设计方案.md`（视觉/页面/接口）+ `mp-prototype/`（6 屏高保真原型 375×812）推进，产品名**羽问**。当前进度 **W1~W4 完成**（微信登录/手机号绑定 + 11 页完整功能 + 内容安全/订阅消息骨架 + tabBar 图标 + 提审清单文档；真机 golden 走查以 `agent_result_w4.json` 为准）。
